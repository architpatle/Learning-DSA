...
/* ---------- INDEX ----------
 ...
 19. Searching Single Element in Sorted Array ( where other Elements are in pair of two [2,2,3,3,5,6,6,8,8])
...
int SingleElementInSortedArray(vector<int> arr)
{
    ...
    while (start <= end)
    {
        ...
        if (mid == arr.size() - 1 && arr[mid] != arr[arr.size() - 2])
        {
            return arr[mid];
        }
...
int main()
{
    vector<int> nums = {2, 5,5, 6, 6, 7, 7, 8, 8};
